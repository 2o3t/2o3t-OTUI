<template>
    <div class="ot-upload vue-image-crop-upload" v-show="value">
        <div class="vicp-wrap">
...
export default {
    ...
    data() {
        ...
        return {
            ...
            isSupported,
            // 浏览器是否支持触屏事件
            isSupportTouch: !$isServer ? document.hasOwnProperty('ontouchstart') : false,
    ...
    methods: {
        ...
        off() {
            if (this.$isServer) return;
            setTimeout(() => {
                this.$emit('input', false); // v-model 事件
        ...
        // 设置图片源
        setSourceImg(file) {
            this.$emit('srcFileSet', file.name, file.type, file.size); // 用户选取文件之后, 参数( fileName, fileType, fileSize )
            const that = this,
                fr = new FileReader();
            fr.onload = function() {
        ...
        prepareUpload() {
            ...
            this.$emit('cropSuccess', createImgUrl, field, ki); // 图片截取完成事件（上传前), 参数( imageDataUrl, field )
            console.log('ccc');
        ...
        upload() {
            ...
            new Promise(function(resolve, reject) {
                ...
                function(resData) {
                    if (that.value) {
                        that.loading = 2;
                        that.$emit('uploadSuccess', resData, field, ki); // 上传成功， 参数( jsonData, field )
                    }
                },
                // 上传失败
                function(sts) {
                    if (that.value) {
                        ...
                        that.$emit('uploadFail', sts, field, ki); // 上传失败， 参数( status, field )
                    }
                }
            );
        },
    ...
    created() {
        if (this.$isServer) return;
        (!this._keyupEvent) && (this._keyupEvent = e => {
